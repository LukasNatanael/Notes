...
from helpers           import relatorios, color_content
...
def vencimento( notes:dict = {}, extra_notes:list = [] ):
    try:
        ...
        extra_info.insert_division('Alteração de vencimento', new_line=2, color=green_text)
        ...
        situação_colorida = color_content.text('Acréscimo', 'red') if ( diferença ) < 0 else color_content.text('Desconto', 'green')
        ...
        infos.write(color_content.text('Mensagem para o cliente:', 'green'), new_line=1)
        infos.write( mensagem_para_cliente )

        infos.write(color_content.text(f'Motivo do {situação.lower()}:', 'green'), new_line=1)
        infos.write(motivo_alteração_valor)

        infos.write(color_content.text('Descrição do atendimento:', 'green'), new_line=1)
